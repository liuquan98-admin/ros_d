...
{
    ros::init(argc,argv,"chao_node"); //初始化chao_node节点
    ros::NodeHandle nh;//创建一个NodeHandle对象与ROS通信
    ros::Publisher pub=nh.advertise<std_msgs::Int32>("/number",10);//创建话题发布者，消息类型Int32,指定缓存大小
    ...
    while (ros::ok())
    {
     ...
     pub.publish(msg);//将发布消息推送到"/number"话题上
